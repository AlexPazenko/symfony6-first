...
class CategoryTreeFrontPage extends CategoryTreeAbstract {
    ...
    public function __construct(EntityManagerInterface $entitymanager, UrlGeneratorInterface $urlgenerator, AppExtension $appExtension) {
      parent::__construct($entitymanager, $urlgenerator);
      $this->slugger = $appExtension;
    }
  ...
  public function getCategoryListAndParent(int $id): string
    {
        ...
        $this->mainParentName = $parentData['name']; // for accesіing in view
        $this->mainParentId = $parentData['id']; // for accesіing in view
        $key = array_search($id, array_column($this->categoriesArrayFromDb, 'id'));
        $this->currentCategoryName = $this->categoriesArrayFromDb [$key]['name']; // for accesіing in view
        ...
        $this->categorylist .= $this->html_1;
            foreach ($categories_array as $value)
            {
              $catName = $this->slugger->slugify($value['name']);
                $url = $this->urlgenerator->generate('video_list.en', ['categoryname' => $catName,
                    'id' => $value['id']]);
